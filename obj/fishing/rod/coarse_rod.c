...
void get_fish(object me)
{
	...
	if( addn_temp("endurance", range_random(-break_int, 1), this_object() ) < 0 )
	{
		...
		set_temp("endurance", copy(query_temp("original_endurance")));
		call_out( (: destruct, this_object(), 1:), 0);
	}	
}
...
void fishing(object me)
{
	...
	me->remove_status(query_temp("last_time_fishing_status", me));
	me->add_status(set_temp("last_time_fishing_status", HIC"釣"NOR CYN"魚"NOR, me), 30);
...
{
	return "、耐久度 "+query_temp("endurance")+"、魚竿等級 "+query("rod_level");
}

void create()
{
	set_idname("coarse rod", "粗製釣竿" );
	
	set_temp("endurance", 3000);
	set_temp("original_endurance", 3000);
