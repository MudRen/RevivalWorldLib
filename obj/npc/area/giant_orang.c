...
([
	500 	:	"巨猩"HIY"獵"NOR YEL"人"NOR,
	5000	:	"巨猩"HIB"剋"NOR BLU"星"NOR,
	50000	:	"巨猩"HIR"殺"NOR RED"手"NOR,
	200000	:	"巨猩"HIM"噬"NOR MAG"者"NOR,
	500000	:	"巨猩"WHT"死"HIW"神"NOR,
]);
...
void reply_defeat(object* attackers)
{
	...
	{
		object ob = new("/quest/monster_fossil/orang_fossil");
		ob->move(this_object());
	}
...
varargs string combat_attack_message(object target, object weapon)
{
	switch(random(5))
	{
		...
		case 1:
			...
			break;
		case 2:
			return "$ME用後腿猛踢$YOU的頭部，";
			break;
		case 3:
			return "$ME從側面快速攻擊$YOU，";
			break;
		case 4:
			return "$ME自樹上撲下攻擊$YOU，";
			break;
	}
}
...
varargs void special_attack(object attacker, object defender)
{
	if( !random(20) )
	{
		msg("$ME抱住$YOU不顧一切地廝咬與獵抓，「"HIR"利爪"NOR YEL"割喉"NOR"」將$YOU狠狠割傷！！！\n", attacker, defender, 1);
		
		COMBAT_D->cause_damage(attacker, defender, range_random(250, 400));
	}
}

void create()
{
	set_idname("giant orang", "巨猩");
	...
	set("abi/stamina/max", 1200);
	set("abi/stamina/cur", 1200);
	set("abi/health/max", 1200);
	set("abi/health/cur", 1200);
	set("abi/energy/max", 1200);
	set("abi/energy/cur", 1200);

	set("skills/unarmed/level", 120);
	set("skills/dodge/level", 120);

	set("long",@TEXT
　　眼前的這頭猩猩明顯的較普通的還要巨大，感覺很不尋常。巨猩以兇惡銳利
...
TEXT);
	...
	if( !random(1000) )
	{
		set_idname(HIY"giant"NOR YEL" orang"NOR, HIY"巨"NOR YEL"猩"NOR);
