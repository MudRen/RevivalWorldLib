	...
	if( option && arg2 )
	{
		...
		switch( lower_case(option) )
		{
			case "author":
			{
				...
				{
					if( !i )
						author = trim(capitalize(multi_author[i]))+NOR"@RevivalWorld\n";
					else
						author += " *          "+trim(capitalize(multi_author[i]))+NOR"@RevivalWorld\n";
				}
				...
				foreach( string ref f in efile )
				{
					if( f[0..10] == " * Author :" )
					{
						f = f[0..10]+" "+trim(kill_repeat_ansi(author));
						break;
					}
				}
			...
			case "update":
			{
				...
				{
					if( !i )
						update += " *  o "+date[6..9]+"-"+date[0..1]+"-"+date[3..4]+" "+capitalize(me->query_id(1))+" "+trim(multi_update[i])+NOR+"\n";		
					else
						update += " *"+repeat_string(" ",16+strlen(me->query_id(1)))+trim(multi_update[i])+NOR+"\n";
				}
				...
				for(int i=0;i<sizeof(efile);i++)
				{
					if( efile[i][0..10] == " * Update :" )
					{
						...
						{
							efile[i+1] = " "+trim(update);
							break;
						}
						
						while( efile[++i] != " *" );
						
						efile = efile[0..i-1] + ({ " "+trim(kill_repeat_ansi(update)) }) + efile[i..];
			...
			case "note":
			{
				...
				foreach( string ref f in efile )
				{
					if( f[0..10] == " * Note   :" )
					{
						f = f[0..10]+" "+trim(kill_repeat_ansi(note));
						break;
					}
				}
