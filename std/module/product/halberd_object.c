...
{
	int faith_halberd_secret_art_1st = query("faith_halberd_secret_art_1st");
	int faith_halberd_secret_art_2nd = query("faith_halberd_secret_art_2nd");
	...
	if( faith_halberd_secret_art_1st > 0 && !random(to_int(500. / calculate_effect(faith_halberd_secret_art_1st))) )
	{
		...
		if( sizeof(weapons) ) 
		{
			...
			weapons[index]->special_attack(attacker, defender);
		}	
		
		if( !objectp(defender) || defender->is_dead() ) return;
	}
	
	if( faith_halberd_secret_art_2nd > 0 && !random(to_int(300. / calculate_effect(faith_halberd_secret_art_2nd))) )
	{
...
string query_description()
{
	...
	if( faith_halberd_secret_art_1st > 0 )
		description += sprintf("  %-20s - 發動機率 "HIY"%.2f%%"NOR, HIG"信念"NOR GRN"之戟"NOR WHT"．"HIW"第一式"NOR, 100./(500./calculate_effect(faith_halberd_secret_art_1st)))+"\n";
		
	if( faith_halberd_secret_art_2nd > 0 )
		description += sprintf("  %-20s - 發動機率 "HIY"%.2f%%"NOR, HIG"信念"NOR GRN"之戟"NOR WHT"．"HIW"第二式"NOR, 100./(300./calculate_effect(faith_halberd_secret_art_2nd)))+"\n";
